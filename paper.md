...
[Priority ranks][26] are the following: lowest, low, medium, high and highest and are defined in an enum. 
I may add more ranks in the future, but it is sufficient for the current needs.

When you mark a handler for export, you can specify a priority rank. 
...
It is easy to forget to call the init function of one of the behaviors and there is no clear indication of that, like a compile time error (you'd most likely just get strange runtime behavior).
...
These values will be gotten by scanning the source code, looking for types or fields marked with the corresponding attributes.

For example, the `Transform` component has an injected field of type `Layers`, which can obviously only have values from the `Layer` enum. 
...
Manual copying is sometimes prefered over automatic code generation, because, for some components, the strategy of creating a copy could be too complex.
So, the code generator will not provide a copy contructor, if it finds a user-defined one.

Ok, now let's understand what is actually going to be copied each time:
1. Values of injected fields. 
They are copied directly (by reference, if it is a reference type, or by making a shallow copy, if it's a value type).
...
This is actually a pretty good idea, although the only benefit one would get from this is that the entity factories would take a little bit less space in memory (the components wouldn't have the runtime fields present).
...
The other components, that the entity of type `A` is not going to have, are `Y` and `Z`.
...
As has been mentioned, defining entity types with JSON files has not been implemented yet, but it is a planned feature.
...
Then, this intermediate representation will either be used directly to generate a factory at runtime, or to generate static classes by means of the code generator.
...
// You cannot define an overload with the same parameters, even if the interfaces are different.
...
Once a mod has been loaded, the id's cannot be reassigned and the types cannot be recreated. 
...
This is bad, because, although we have just one mod, mod `B`, if we have had only `B` in the first place, the id's would have been different, because the types would have had the id's from 0 through 3 instead. 
...
Obviously, if the number of currently registered flags is over the size of the integer, no new flags could be added, but I don't think that would ever happen.
...
- Writing my own analyser framework is a lot more work than even writing my own templating engine;
...
It inserts all of this printing code into the overridden `TransformText()` method.
...
See [this stackoverflow issue where I shared my solution][54].
...
In particular, the pattern here is getting a component from an entity, and immediately calling one of its methods with certain arguments, possibly passing the entity itself to it as an argument.
...
So, in order to activate the `Moving` behavior, that is, call this method correctly, you'd have to write code like this:
```C#
